<script setup lang="ts">
    const post00 = {
        ...
        description: 
            "Tuzzin Leadership (Mr. Jemin and Mr. Dipen) appreciated by NPCI for their Product: YoLokal (YoLokal.com)- Card Linked Offers and CashbackFrom NPCI Team: Thank you for your stimulating presentation at yesterday’s FINSPIRE 3 workshop for the Cooperative Banks of Maharashtra. We are grateful for the time and effort you took to share your expertise and ideas to the banks and address their queries. The workshop became a great success with a large group of banks participating in it and with the valuable contribution from you and many other expert speakers.Looking forward for many such collaborations.Tuzzin thanks NPCI for their great support and look forward to assist NPCI in any way possible."
    };
    ...
    const post02 = {
        ...
        description: 
            "Work Life Balance is critical for productivity. Exercise plays a crucial role in unwinding from work and helping with health.Tuzzin has started special incentive of additional bonus to employees hitting the Gym. Every employee will get special bonus if they regularly go to Gym. Tuzzin is committed to employee health and is proud of introducing such an initiative."
    };
...
<template>
    <div class="rounded-lg shadow-lg overflow-hidden flex flex-col md:flex-row mx-auto py-4 max-w-5xl mt-3" >
        ...
        <img
            src="/images/NPCI-Appreciation.jpg"
            alt="Post Image"
            class="w-full h-auto object-cover"
        />
        </div>
        <div class="w-full md:w-2/3 px-6 py-4">
        <h2 class="text-2xl font-bold mb-2 text-primary">{{ post00.title }}</h2>
        <p class="text-muted text-base mb-4">
            <!-- {{ post.description}} -->
            {{ truncateDescription(post00.description, 30) }}
        </p>
        <NuxtLink
            :to="'/PostDetail'"
    ...
    <div class="rounded-lg shadow-lg overflow-hidden flex flex-col md:flex-row mx-auto py-4 max-w-5xl mt-3">
        <div class="w-full md:w-1/3 p-4">
        <img
            src="/images/featured-news-06.jpg"
            alt="Post Image"
            class="w-full h-auto object-cover"
        />
        </div>
        <div class="w-full md:w-2/3 px-6 py-4">
        <h2 class="text-2xl font-bold mb-2 text-primary">{{ post01.title }}</h2>
        <p class="text-muted text-base mb-4">
            <!-- {{ post.description}} -->
            {{ truncateDescription(post01.description, 30) }}
        </p>
        <NuxtLink
            :to="'/PostDetail-01'"
            class="bg-slate-500 hover:bg-slate-700 text-white font-bold py-2 px-4 rounded"
    ...
    <div class="rounded-lg shadow-lg overflow-hidden flex flex-col md:flex-row mx-auto py-4 max-w-5xl mt-3">
        ...
        <div class="w-full md:w-2/3 px-6 py-4">
        <h2 class="text-2xl font-bold mb-2 text-primary">{{ post02.title }}</h2>
        <p class="text-muted text-base mb-4">
            <!-- {{ post.description}} -->
    ...
    <div class="rounded-lg shadow-lg overflow-hidden flex flex-col md:flex-row mx-auto py-4 max-w-5xl mt-3">
        ...
        <div class="w-full md:w-2/3 px-6 py-4">
        <h2 class="text-2xl font-bold mb-2 text-primary">{{ post03.title }}</h2>
        <p class="text-muted text-base mb-4">
            <!-- {{ post.description}} -->
            {{ truncateDescription(post03.description, 30) }}
        </p>
        <NuxtLink
           :to="'/PostDetail-03'"
            class="bg-slate-500 hover:bg-slate-700 text-white font-bold py-2 px-4 rounded"
    ...
    <div class="rounded-lg shadow-lg overflow-hidden flex flex-col md:flex-row mx-auto py-4 max-w-5xl mt-3">
        ...
        <div class="w-full md:w-2/3 px-6 py-4">
        <h2 class="text-2xl font-bold mb-2 text-primary">{{ post04.title }}</h2>
        <p class="text-muted text-base mb-4">
            <!-- {{ post.description}} -->
            {{ truncateDescription(post04.description, 30) }}
        </p>
        <NuxtLink
           :to="'/PostDetail-04'"
            class="bg-slate-500 hover:bg-slate-700 text-white font-bold py-2 px-4 rounded"
    ...
    <div class="rounded-lg shadow-lg overflow-hidden flex flex-col md:flex-row mx-auto py-4 max-w-5xl mt-3">
        ...
        <div class="w-full md:w-2/3 px-6 py-4">
        <h2 class="text-2xl font-bold mb-2 text-primary">{{ post05.title }}</h2>
        <p class="text-muted text-base mb-4">
            <!-- {{ post.description}} -->
            {{ truncateDescription(post05.description, 30) }}
        </p>
        <NuxtLink
            :to="'/PostDetail-05'"
            class="bg-slate-500 hover:bg-slate-700 text-white font-bold py-2 px-4 rounded"
